	...
	char* fmt;int a1, a2, a3, a4, a5, a6, a7, a8, a9;
{
	...
	add_message_calls++; /* We want to know how many packets the old
		...
		(void) sprintf(buff + old_message_length, fmt, a1, a2, a3, a4, a5, a6,
				a7, a8, a9);
	...
	if (timer_expired()) {
		...
		{ /* waiting packets */
			...
			continue;
					...
					ip->text_end += copy_chars(buff, ip->text + ip->text_end,
							l);
#if 0
		    for (x=0; x<l; x++) {
			if (buff[x] == '\n' || buff[x] == '\r')
	...
	int i;
{
	...
	fatal("Get interactive: player %d not found! (num_players = %d)", i,
			num_player);
	...
	int new_socket;struct sockaddr_in* addr;int len;
{
	...
	for (i = 0; i < MAX_PLAYERS; i++)
	{
		...
		master_ob->interactive = (struct interactive*) xalloc(
				sizeof(struct interactive));
