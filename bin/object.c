	...
	struct vector *v;
{
	...
	for (i = 0; i < v->size; i++)
	{
		...
		{
			strcat(buf, "\"");
			my_strcat(buf, v->item[i].u.string); /* my_ */
			strcat(buf, "\",");
		}
		else if (v->item[i].type == T_POINTER)
		{
		...
		else if (v->item[i].type == T_NUMBER)
		{
			...
			strcat(buf, "0,"); /* Objects can't be saved. */
	}
	strcat(buf, "})");
	return buf;
}
	...
	struct object *ob;char *from;
{
	...
	if (ob->prog)
	{
		tot_alloc_object_size -= (ob->prog->num_variables - 1)
				* sizeof(struct svalue) + sizeof(struct object);
		free_prog(ob->prog, 1);
		ob->prog = 0;
	}
	if (ob->swap_num != -1)
...
void reference_prog(progp, from)
	struct program *progp;char *from;
{
	progp->ref++;
	if (d_flag)
...
void free_prog(progp, free_sub_strings)
	struct program *progp;int free_sub_strings;
{
	progp->ref--;
	if (progp->ref > 0)
		return;
