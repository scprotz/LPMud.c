...
void load_first_objects(), prepare_ipc(), shutdowngame(), ed_cmd(char*),
		print_prompt(), call_out(), destruct2(struct object*);

extern int get_message(char*, int);
extern int player_parser(char*);
extern int call_function_interactive(struct interactive*, char*);
//extern int resort_free_list();
extern int swap(struct object*);
...
static void look_for_objects_to_swap()
{
	...
	memcpy((char*) save_error_recovery_context, (char*) error_recovery_context,
	...
	if ((num_player > 0) && hb_list)
	{
				...
				&& (num_done < num_hb_objs))
		{
			...
			if (ob->user)
				ob->user->heart_beats++;
			eval_cost = 0;
			call_function(ob->prog, &ob->prog->functions[ob->prog->heart_beat]);
		}
		if (num_hb_objs)
...
{ /* Old version used when o_flag true /JnA */
	...
	resort_free_list();
		...
		times(&tms2);
		(void) printf(" %.2f\n",
				(tms2.tms_utime - tms1.tms_utime + tms2.tms_stime
						- tms1.tms_stime) / 60.0);
		tms1 = tms2;
#else
