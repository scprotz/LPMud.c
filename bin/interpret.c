	...
	int push;
{
	...
	if (push == 1)
	{
		...
		memcpy((char*) p->old_error_context, (char*) error_recovery_context,
	...
	else
	{
		...
		memcpy((char*) error_recovery_context, (char*) p->old_error_context,
				sizeof error_recovery_context);
	...
	char* p;
{
	...
	csp->extern_call = 1;
	pc = p;
	again: instruction = EXTRACT_UCHAR(pc);
#ifdef TRACE_CODE
	...
	if (instrs[instruction].min_arg != instrs[instruction].max_arg)
	{
		...
		if (num_arg > 0)
		{
			if (instrs[instruction].type[0] != 0
					&& (instrs[instruction].type[0] & (sp - num_arg + 1)->type)
							== 0)
			{
		...
		if (num_arg > 1)
		{
			if (instrs[instruction].type[1] != 0
					&& (instrs[instruction].type[1] & (sp - num_arg + 2)->type)
							== 0)
			{
	...
	switch (instruction)
	{
		...
		CASE(F_IDENTIFIER)
			...
			assign_svalue_no_free(sp,
					find_value(
							(int) (EXTRACT_UCHAR(pc) + variable_index_offset)));
			pc++;
			/*
			 * Fetch value of a variable. It is possible that it is a variable
			 * that points to a destructed object. In that case, it has to
	...
	if (current_object == master_ob)
		...
		CASE(F_AND)
			...
			{
				extern struct vector* intersect_array(struct vector*,
						struct vector*);
		...
		CASE(F_MEMBER_ARRAY)
			;
			{
				...
				for (i = 0; i < v->size; i++)
				{
					...
					{
						case T_STRING:
							if (strcmp((sp - 1)->u.string, v->item[i].u.string)
									== 0)
								break;
							continue;
						case T_POINTER:
	...
	if (!command_giver) {
		...
		CASE(F_ADD_ACTION)
			;
			{
				...
				add_action(arg[0].u.string, num_arg > 1 ? arg[1].u.string : 0,
			...
			switch (argp->type)
			{
				case T_STRING:
				{
					...
					else if (sp->type == T_NUMBER)
					{
						...
						sprintf(buff, "%d", sp->u.number);
						new_str = xalloc(
								strlen(argp->u.string) + strlen(buff) + 1);
						strcpy(new_str, argp->u.string);
						strcat(new_str, buff);
						pop_n_elems(2);
						push_malloced_string(new_str);
	...
	path = check_valid_path(sp->u.string, current_object->eff_user, "cindent", 1);
		...
		CASE(F_DESCRIBE)
			;
			{
				...
				pop_n_elems(2);
				if (str)
					push_malloced_string(string_copy(str));
				else
					push_number(0);
				break;
			}
		...
		CASE(F_SORT_ARRAY)
			;
			{
				...
				if (arg[0].type == T_POINTER)
				{
					...
					res = sort_array(
							slice_array(arg[0].u.vec, 0,
									arg[0].u.vec->size - 1), arg[1].u.string,
							ob);
				}
				else
					res = 0;
		...
		CASE(F_ORDER_ALIST)
			;
			{
				...
				for (i = 0; i < listsize; i++)
				{
					...
					list->item[i].u.vec = slice_array(args[i].u.vec, 0,
							keynum - 1);
				}
				pop_n_elems(num_arg);
				sp++;
				...
				else
				{
					...
					if (args[1].u.vec->size > 1)
					{
						if (args[1].u.vec->item[1].type != T_POINTER
								|| args[1].u.vec->item[1].u.vec->size
										!= keys->size)
						{
							bad_arg(2, F_ASSOC);
						}
			...
			;
			{
				...
				switch (arg[0].u.number)
				{
					case 0:
					{
						...
						for (obj2 = obj_list, i = 1; obj2; obj2 =
								obj2->next_all, i++)
							if (obj2->next_all == ob)
							{
								add_message(
	...
	char* fun;struct object* ob;int num_arg;
{
	...
	while (ob->shadowed && ob->shadowed != current_object)
		ob = ob->shadowed;
	retry_for_shadow: if (ob->flags & O_SWAPPED)
	...
	int how;
{
	...
	for (p = &control_stack[0]; p < csp; p++)
	{
		(void) printf("'%15s' in '%20s' ('%20s')line %d\n",
				p[0].funp ? p[0].funp->name : "CATCH", p[1].prog->name,
	...
	(void) printf("'%15s' in '%20s' ('%20s')line %d\n",
			p[0].funp ? p[0].funp->name : "CATCH", current_prog->name,
	...
	do
	{
		if (previous_instruction[i] != 0)
			printf("%6p: %3d %8s %-25s (%d)\n", previous_pc[i],
					...
					get_f_name(previous_instruction[i]), stack_size[i] + 1);
