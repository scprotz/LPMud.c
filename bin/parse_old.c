...
void fixlist(first, parent, antal)
	OBJECT *first;struct altern_objects **parent;int *antal;
{
	OBJECT *cur;
	...
	OBJECT *obj;
{
	...
	if (gDebug)
		fprintf(stderr, "Putting: %s", obj->name);
	ao = gPobjects;
	old = ao;
	while (ao)
	{
	...
	OBJECT *on;char *apa;
{
	...
	if (gDebug)
		fprintf(stderr, "Calling %s in %s with %s\n", fnamn, on->name, apa);
	...
	char *str;
{
	...
	sing[sl - 3] = 0;
	if (gDebug)
		fprintf(stderr, "To singular (%s) end(%s) begin(%s)\n", str, sp, sing);
	if (EQ(str, "corpses"))
		return "corpse";
	if (EQ(sp, "ses"))
		return strcat(sing, "s");
	else if (EQ(sp, "xes"))
		return strcat(sing, "x");
	else if (EQ(sp, "hes"))
		return strcat(sing, "h");
	else if (EQ(sp, "ies"))
		return strcat(sing, "y");
	else if (EQ(sp, "ves"))
		return strcat(sing, "fe");
	else if (sp[2] == 's')
	{
		strcat(sing, sp);
		sing[sl - 1] = 0;
	...
	char *onam;int plur;
{
	...
	static char *plurpron[] =
	{ "those", "them", "these", "ones", "$" };
	static char *singpron[] =
	{ "this", "one", "it", "$" };
	int ilop;
	...
	char **cmd;int plur;
{
	OBJECT *on;
	struct altern_objects *ao;
	char *st, *ocmd, totname[4 * KLUDGELEN], tot2[4 * KLUDGELEN];
	...
	while (st[0])
	{
		...
		while (ao)
		{
			...
			if (plur)
			{
				if (call_obj(QPLURIDFUNC, ao->ao_obj, totname))
					on = ao->ao_obj;
				else if (call_obj(QIDFUNC, ao->ao_obj, tot2))
					on = ao->ao_obj;
			}
			else if (call_obj(QIDFUNC, ao->ao_obj, totname))
				on = ao->ao_obj;
			if (on)
				italt_put(on);
			ao = ao->next;
	...
	char **cmd;int plur;
{
	...
	{
		if (gDebug)
			fprintf(stderr, "Skip 'the'\n");
		*cmd = ocmd;
		getfirst(cmd);
		if ((pn = finditem(cmd, plur)))
			return pn;
		else
		{
			*cmd = ocmd;
	...
	else
	{
		*cmd = ocmd;
		...
		if (gDebug)
			fprintf(stderr, "Done matchobject in finditem.\n");
	}

	if (pn == 0)
		*cmd = ocmd;
	if (gDebug)
		fprintf(stderr, "Done finditem.\n");
	return pn;
}
	...
	if (finditem(cmd, 0))
	{
		if (gCarg)
		{
			...
			for (s = 1; s <= nm; s++)
			{
				...
				if (!ob)
					if (gDebug)
						fprintf(stderr, "No object from italt %d\n", s);
				if (ob)
					add_ref(ob, "parse_command");
			}
	...
	VALUE *v;
{
	...
	getfirst(cmd);
	if (!f)
		return 1; /* Match and word in first element */
	/*
	...
	char **cmd;
{
	...
	{ "on", "in", "under", "from", "behind", "beside", 0 };
	SVALUE *v;
	int cnt;
	...
	LVALUE **lin;int skip;
{
	...
	switch (ch)
	{
		...
		case '\'':
			...
			break;
		case '[':
			sscanf(cod, "\[%[^\135]", gMword);
			pt = OTX; /* 135 is oct for ] */
			if (gMword[strlen(gMword) - 1] == ']')
				gMword[strlen(gMword) - 1] = 0;
			break;
		case '/':
			pt = ALT;
			break;
	...
	if (gDebug)
		fprintf(stderr, "(%s,%s,%d,%c)", cod, gMword, pt, ch);
	gCarg = 0;
...
int parse(cs, ob_or_array, ps, dest_args, num_arg)
	char *cs, *ps;struct svalue *ob_or_array, *dest_args;int num_arg;
{
	/* Arguments:
	 string      Command
	...
	gPobjects = gOblist;
	if (gDebug)
		fprintf(stderr, "Has made list of Acsobjects. %d \n", itnumalt());
	if (gDebug)
		fprintf(stderr, "Parsing: %s, with pattern: %s\n", cmd, parsep);
	gPobjects = 0;
	...
	while (ptyp != EP)
	{
		...
		altflag = 0;
		switch (ptyp)
		{ /* See which pattern type to search for */
		 ...
		 */
		if (altflag)
		{ /* Pattern matched, fetch next and end string input */
			if (ptyp != TX)
				txflag = -1; /* End string input if not just started */
			ptyp = get1ps(&parsep, &l, 0);
			while (ptyp == ALT)
			{ /* Skip left over alternatives */
		...
		if (gDebug)
			fprintf(stderr, "Pattern, after: %d\n", ptyp);
	}

	/* End of pattern reached, what have got? What is left?
	 */
	if (gDebug)
		fprintf(stderr, "Pattern ended\n");

	if (txflag >= 0)
	{ /* It ended with a %s, fill up the string */
		while (!EMPTY(cmd))
			addword(tx_jp, getfirst(&cmd));
	}

	if (!EMPTY(cmd))
		ptyp = ALT; /* No match so set ptyp != EP */
	...
	free(ops);
	if (gPobjects)
		italt_new(); /* Free alternate object list */
	if (gOblist)
	{
		gPobjects = gOblist;
		italt_new(); /* Free list of accessible objects */
	}
	if (ptyp == EP)
	{
		if (gDebug)
			fprintf(stderr, "Exiting Jnaparse with match\n");
		return 1; /* Successfull match */
	}
	else
	{
		if (gDebug)
			fprintf(stderr, "Exiting Jnaparse: No match\n");
		return 0; /* No match */
	}
}
