	...
	char *buff;
{
	...
	for (s = command_giver->sent; s; s = s->next)
	{
		...
		if (s->verb == 0)
			error(
					"An 'action' did something, but returned 0 or had an undefined verb.\n");
		len = strlen(s->verb);
		if (s->no_space)
		{
	...
	char *buff;
{
	...
	if (strcmp(buff, "status") == 0 || strcmp(buff, "status tables") == 0)
	{
		...
		extern int tot_alloc_sentence, tot_alloc_object, tot_alloc_object_size,
				num_swapped, total_bytes_swapped, num_arrays, total_array_size;
	...
	if (verbose)
		...
		if (!verbose)
		{
			...
			add_message("Prog blocks:\t\t\t%8d %8d (%d swapped, %d Kbytes)\n",
					total_num_prog_blocks, total_prog_block_size, num_swapped,
					total_bytes_swapped / 1024);
			add_message("Memory reserved:\t\t\t %8d\n", res);
		}
		if (verbose)
			stat_living_objects();
		tot = total_prog_block_size + total_array_size + tot_alloc_object_size
				+ show_otable_status(verbose) + heart_beat_status(verbose)
				+ add_string_status(verbose) + print_call_out_usage(verbose)
				+ res;
	...
	char *fmt;int a, b, c, d, e, f, g, h;
{
	...
	if (current_object)
	{
		...
		if (current_prog)
			save_error(emsg_buf, current_prog->name, get_line_number_if_any());
	}
...
char* check_valid_path(path, eff_user, call_fun, writeflg)
	char *path;struct wiz_list *eff_user;char *call_fun;int writeflg;
{
	struct svalue *v;
	...
	int minutes;
{
	...
	if (!ob)
	{
		struct object *save_current = current_object, *save_command =
				command_giver;
		command_giver = 0;
		current_object = 0;
